...
Error FrameBuffer::Initialize(const FrameBufferConfig& config) {
	...
	// 対応していないデータ形式の場合にエラーを返す
	if (bits_per_pixel <= 0) {
		return MAKE_ERROR(Error::kUnknownPixelFormat);
	}

	// 特別な場合を除きconfig_.frame_bufferはnullptrとなっていて、else節が実行されることを想定している
	if (config_.frame_buffer) {
		buffer_.resize(0);
	} else {
		// 計算で得たビット数を用いて描画領域に必要なバイト数を求める
		buffer_.resize(
			((bits_per_pixel + 7) / 8)
...
Error FrameBuffer::Copy(Vector2D<int> pos, const FrameBuffer& src) {
	// コピー元(src)とコピー先(this)のピクセルのデータ形式が同じかどうかを調べる
	...
	// posで指定した位置が範囲外だったり、srcのサイズが大きくてはみ出したりする場合に対応するための計算
	...
	// src_bufからdst_bufへコピーする処理を列の数だけ繰り返す
